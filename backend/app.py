...
class CourseProviderUnavailable(Exception):
    def __init__(self, message, status_code=501):
        super().__init__(message)
        self.status_code = status_code
    ...
    try:
        ...
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS courses (
                ...
                school TEXT NOT NULL DEFAULT 'uoft',
                code TEXT NOT NULL,
        ...
        conn.execute(
            f'''
            ALTER TABLE courses
            ADD COLUMN IF NOT EXISTS school TEXT NOT NULL DEFAULT '{DEFAULT_SCHOOL}'
            '''
        )
        conn.execute(
            'UPDATE courses SET school = %s WHERE school IS NULL OR BTRIM(school) = %s',
            (DEFAULT_SCHOOL, ''),
        )
        conn.execute(
        ...
        conn.execute(
            'CREATE INDEX IF NOT EXISTS idx_courses_user_school ON courses (user_id, school)'
        )
        conn.execute(
...
# Delete one user's saved courses for a school and all related child records.
def clear_saved_courses(conn, user_id, school):
    # Child rows are removed automatically because the tables use ON DELETE CASCADE.
    conn.execute(
        'DELETE FROM courses WHERE user_id = %s AND school = %s',
        (user_id, school),
    )
...
def normalize_school_value(value, fallback=DEFAULT_SCHOOL):
    normalized = str(value or '').strip().lower()
    if not normalized:
        return fallback

    school = SCHOOL_ALIASES.get(normalized, normalized)
    if school in VALID_SCHOOLS:
        return school
    return fallback


def get_requested_school(value):
    ...
    return None, 'School must be one of: uoft, mac.'


def get_school_label(school):
    return SCHOOL_LABELS.get(normalize_school_value(school), SCHOOL_LABELS[DEFAULT_SCHOOL])
...
def validate_course_payload(courses, school=DEFAULT_SCHOOL):
    if not isinstance(courses, list):
        return 'Courses payload must be a list.'

    active_school = normalize_school_value(school)
    ...
    for course in courses:
        ...
        course_school = normalize_school_value(course.get('school'), active_school)
        if not course_code:
            return 'Every course needs a code.'
        if course_school != active_school:
            return (
                f'Course "{course_code}" belongs to {get_school_label(course_school)}, '
                f'but this save is for {get_school_label(active_school)}.'
            )

        course_key = (course_school, course_code)
...
class CourseProvider:
    ...
    label = SCHOOL_LABELS[DEFAULT_SCHOOL]

    def lookup_course(self, course_code, section_code=''):
        raise NotImplementedError

    def lookup_suggestions(self, query, limit=MAX_COURSE_SUGGESTIONS):
        raise NotImplementedError

    def with_school_metadata(self, payload):
        enriched = dict(payload)
        enriched['school'] = self.school
        enriched['schoolLabel'] = self.label
        return enriched


class UofTCourseProvider(CourseProvider):
    school = 'uoft'
    label = SCHOOL_LABELS['uoft']

    def lookup_course(self, course_code, section_code=''):
        course = lookup_ttb_course(course_code, section_code)
        return self.with_school_metadata(course) if course else None

    def lookup_suggestions(self, query, limit=MAX_COURSE_SUGGESTIONS):
        return [
            self.with_school_metadata(suggestion)
            for suggestion in lookup_course_suggestions(query, limit)
        ]


class McMasterCourseProvider(CourseProvider):
    school = 'mac'
    label = SCHOOL_LABELS['mac']
    unavailable_message = (
        'McMaster course search is not connected yet. McMaster points students '
        'to MyTimetable for public timetable search, and its API developer '
        'portal requires MacID sign-in.'
    )

    def lookup_course(self, course_code, section_code=''):
        raise CourseProviderUnavailable(self.unavailable_message)
...
COURSE_PROVIDERS = {
    'uoft': UofTCourseProvider(),
    'mac': McMasterCourseProvider(),
}


def get_course_provider(school):
    return COURSE_PROVIDERS[normalize_school_value(school)]


def get_courses_by_school(school, query='', limit=MAX_COURSE_SUGGESTIONS):
    provider = get_course_provider(school)
    return provider.lookup_suggestions(query, limit)


# Render the timetable builder as the public landing page.
@app.route('/')
def home():
...
# Render the timetable builder for guests and signed-in users.
@app.route('/index')
def index():
    return render_template('index.html')


# Report whether the current browser has an authenticated account session.
...
# Clear the current session and keep the user on the public builder.
@app.route('/logout', methods=['POST'])
def logout():
    session.clear()
    return {'message': 'Logged out', 'redirect_url': url_for('home')}
...
def get_courses():
    ...
    school, error_message = get_requested_school(request.args.get('school'))
    if error_message:
    ...
    courses = conn.execute(
        'SELECT * FROM courses WHERE user_id = %s AND school = %s ORDER BY id',
        (session['user_id'], school),
    ).fetchall()
    conn.close()
    return [
        {
            'id': c['id'],
            'school': c.get('school') or school,
            'code': c['code'],
...
def save_courses():
    ...
    school, error_message = get_requested_school(req_data.get('school'))
    if error_message:
        return {'error': error_message}, 400

    courses = req_data.get('courses', [])
    error_message = validate_course_payload(courses, school)
    if error_message:
        return {'error': error_message}, 400

    ensure_db_initialized()
    conn = get_db_connection()
    clear_saved_courses(conn, session['user_id'], school)

    for course in courses:
        course_row = conn.execute(
            'INSERT INTO courses (user_id, school, code, color_index) VALUES (%s, %s, %s, %s) RETURNING id',
            (
                session['user_id'],
                school,
...
def search_ttb_course():
    ...
    provider = get_course_provider(school)
    try:
        course = provider.lookup_course(course_code, section_code)
    except CourseProviderUnavailable as exc:
        return {'error': str(exc), 'school': school}, exc.status_code
    ...
    if not course:
        return {
            'error': (
                f'Could not find {get_school_label(school)} course '
                f'"{course_code}".'
            )
        }, 404
...
def get_course_suggestions():
    ...
    if len(query) < 2:
        return {'suggestions': [], 'school': school}

    provider = get_course_provider(school)
    try:
        suggestions = provider.lookup_suggestions(query, limit)
    except CourseProviderUnavailable as exc:
        return {'suggestions': [], 'school': school, 'message': str(exc)}
    except CourseSuggestionError as exc:
        response_body = {'error': str(exc)}
        if exc.details:
            response_body['details'] = exc.details
    ...
    return {'suggestions': suggestions, 'school': school}
